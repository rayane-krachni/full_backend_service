...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const uploadPath = path.join(process.cwd(), 'data', 'static', 'public', 'organizations');
    ...
    try {
        ...
        if (req.file && req.file.fieldname === "picture") {
            try {
                ...
                const fileLink = `${req.protocol}://${req.get('host')}/data/static/public/organizations/${req.file.filename}`;
