...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const uploadPath = path.join(process.cwd(), 'data', 'static', 'public', 'reasons');
    ...
    try {
        ...
        if (req.file && req.file.fieldname === "picture") {
            try {
                ...
                const fileLink = `${req.protocol}://${req.get('host')}/data/static/public/reasons/${req.file.filename}`;
